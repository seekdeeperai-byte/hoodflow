...
/**
 * Schema bootstrap DDL, kept in sync with apps/api/migrations/001_init.sql by
 * `postgres-history-store.test.ts` (the test fails the build if the two ever
 * drift). The .sql file remains the human/psql/Supabase-facing source of
 * truth and carries the full design rationale; this constant exists because
 * reading it from disk at runtime is not portable.
 *
 * Previously `ensureSchema()` did `readFileSync(join(__dirname, "..", "..",
 * "migrations", "001_init.sql"))`. That works from a `tsc` build tree but
 * breaks the moment the app is bundled — a serverless deployment (the Vercel
 * target) relocates the compiled output and does not carry sibling .sql files
 * into the bundle unless explicitly configured, so the first database call
 * would throw ENOENT instead of creating the table. Inlining the statements
 * removes a filesystem dependency from a code path that only ever needed a
 * string.
 */
const SCHEMA_DDL = `
